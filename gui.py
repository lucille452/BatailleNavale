...
import random  # Ajouté pour la fonction adversaire_tirer
...
class GameGUI:
    def __init__(self, root):
        ...
        self.game = Game()
        self.orientation_var = tk.StringVar(value="horizontal")  # Déplacement de cette ligne ici pour éviter l'erreur
        self.navires_a_placer = self.game.ships.copy()
        self.navires_places = []
        self.tirs_effectues = []
        self.create_widgets()

    def create_widgets(self):
        self.tir_frame = tk.Frame(self.root)
        ...
        self.navires_joueur_frame = tk.Frame(self.root)
        ...
        self.place_navires_button = tk.Button(self.root, text="Placer Navires", command=self.toggle_place_navires_mode)
        self.place_navires_button.grid(row=2, column=0, padx=20, pady=10)

        self.orientation_menu = tk.OptionMenu(self.root, self.orientation_var, "horizontal", "vertical")
        self.orientation_menu.grid(row=3, column=0, padx=20, pady=10)

        self.navire_var = tk.StringVar()
        self.navire_combobox = ttk.Combobox(self.root, textvariable=self.navire_var)
    ...
    def create_grille(self, parent, label_text):
        ...
        for row in range(10):
            button_row = []
            for col in range(10):
                button = tk.Button(parent, text=' ', width=2, height=1, bg='light blue',
                                   command=lambda r=row, c=col: self.on_button_click(parent, r, c))
    ...
    def toggle_place_navires_mode(self):
        self.place_navires_mode = not getattr(self, 'place_navires_mode', False)
